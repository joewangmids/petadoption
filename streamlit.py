...
def generate_full_dashboard_html(pet_data):
    ...
    if pd.notna(recommended_team):
        team_html_module = f"""<div class="team-section"><div class="team-header"><div class="team-avatar"><i class="fas fa-hands-helping"></i></div><div class="team-info"><h3>{recommended_team}</h3><div class="team-title">Predicted Outcome</div></div></div></div>"""
    ...
    <head><meta charset="UTF-8"><script src="https://cdn.tailwindcss.com"></script><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    ...
    <body><div class="bg-white p-4 sm:p-6"><div class="mb-3"><h1 class="text-2xl font-bold text-gray-800">Pet Adoptability Dashboard</h1><p class="text-sm text-gray-500">Pet ID: #{pet_id}</p></div>
    <div class="flex flex-col gap-4 max-w-3xl mx-auto"><div class="bg-gray-50 rounded-lg p-4 shadow-sm module"><h2 class="text-lg font-bold text-gray-700 mb-2">Adoption Score</h2><h3 class="font-bold text-gray-700">Score: {score}</h3><div class="progress-bar mt-1"><div class="progress-fill {progress_color}" style="width:{score}%"></div></div><div class="mt-3"><span class="{progress_color} text-white px-3 py-0.5 rounded-full text-sm font-medium">{risk_category}</span></div></div>
    {team_html_module}<div class="bg-gray-50 rounded-lg p-4 shadow-sm module"><h2 class="text-lg font-bold text-gray-700 mb-2">Top Factors Affecting Score</h2><div class="space-y-2">{factors_html}</div></div></div></div></body></html>
    """
...
if df is not None:
    ...
    # --- CHANGE: Case updated for all column names ---
    ...
    with col2:
        ...
        if st.session_state.selected_pet_id:
            # --- CHANGE: Case updated to find selected pet ---
